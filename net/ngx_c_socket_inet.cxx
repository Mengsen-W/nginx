...
/*
 * @ Description: 将socket绑定的地址转换位文本格式
 * @ Parameters: struct sockaddr *sa, int port, u_char *text, size_t leftlen
 * @ Return size_t
 */
size_t CSocket::ngx_sock_ntop(struct sockaddr *sa, int port, u_char *text,
                              size_t len) {
  ...
  switch (sa->sa_family) {
    case AF_INET:
      ...
      if (port) { /* 端口信息也组合到字符串里 */
        ...
        p = ngx_snprintf(text, len, "%ud.%ud.%ud.%ud:%d", p[0], p[1], p[2],
                         p[3], ntohs(sin->sin_port)); /* 返回的是新的可写地址 */
      } else /* 不需要组合端口信息到字符串中 */
      {
